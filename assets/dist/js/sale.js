var mainFunction = (function() {
	let main = {
		...
		ready: function() {
			...
			$(document).ready(function() {
				...
				$(this).on('click', "#add", function() {
					...
					lastTr.find('.rowid,.raw_material_string,.gross_weight,.less_weight,.net_weight,.touchData,.wastage,.piece,.labour,.rate,.sub_total,.fine,.other_amount').val(0);
				...
				$(document).on('focus', '.gross_weight,.less_weight, .touchData, .wastage, .quantity, .rmrate,.labour,.other_amount,.piece,.rate', function() {
					main.handleInputFocusAndBlur(this, 'focus');
				}).on('blur', '.gross_weight,.less_weight, .touchData, .wastage, .quantity, .rmrate,.labour,.other_amount,.piece,.rate', function() {
				...
				$(this).on('input', '.gross_weight,.touchData,.wastage,.piece,.less_weight,.rate,.sub_total,.labour, .gross_weight, .piece, .other_amount', function() {
					main.calculateMain($(this));
					main.Totalcalculate();
				...
				$(this).on('change', '.labour, .gross_weight, .piece, .other_amount,.labour_type', function() {
					main.calculateLabour($(this));
				});
	    ...
	    setCustomerData: function(row,data) {
	        if(data != "" && data != null){
    // 			row.children('td').find(".stamp").val(data.).trigger("change");
    			row.children('td').find(".labour").val(data.rate);
    			row.children('td').find(".labour_type").val(data.label.toLowerCase()).trigger("change");
        ...
        getTagCustomerData : function(Tag){
            if (Tag) {
    			if ($('#party_id').val() > 0) {
    				...
    				$.ajax({
    					url:`${BaseUrl}/sales/receiveTag/${Tag}`,
    					success:async function (data) {
    						...
    						if (data.success) {
    							...
    							$('.append-here').children('tr').each(function (i, v) {
    								if ($(v).data('tag') == Tag) {
    							...
    							if (isTagExistsInList) {
                				    ...
                				    await main.getCustomerData(customer_id, item_id);
                				    $('.append-here').children('tr').not(":first").remove();
                				    
                				    if(main.customerData != "" && main.customerData != null){
                                        main.customerData.forEach(function(v, i) {
                                            let row;
                                            if (i === 0) {
                                            ...
                                            main.setCustomerData(row,v); // Pass the current item v instead of the whole array
                                            main.setLotData(row, lotData);
                                            row.find('.item').val(item_id).trigger('change');
                                            
                                            main.calculateMain(row.children('td'));
                                			main.Totalcalculate();
                                			main.calculateLabour(row.children('td'));
                                        });
                				    }else{
                                        row = $('.append-here').children('tr:last');
                                        main.setCustomerData(row,main.customerData); // Pass the current item v instead of the whole array
                                        var isLotDataSet = main.setLotData(row, lotData);
                                        row.find('.item').val(item_id).trigger('change');
		...
		calculateMain: function(ref) {
			...
			if(container.find(".rate").val() > 0){
			    ...
			    container.find(".fine").val(formatNumber(parseFloat(fine) * parseFloat(net_weight) / 100));
			}
			main.Totalcalculate();
		},
		...
		calculateLabour: function(ref) {
			...
			// if (labour_type === "") {
			// 	container.find(".labour").val("0");
			// 	// container.find(".labour_type ").select2("open");
			// }
		...
		handleInputFocusAndBlur: function(element, eventType) {
			var $element = $(element);
			if (eventType === 'focus' && $element.val() == '0' || $element.val() == "0.00") {
				$element.val('');

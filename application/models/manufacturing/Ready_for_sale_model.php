<?php
class Ready_for_sale_model extends CI_Model
{
	...
	function getSalesGroupByItem($filterParams)
	{
		...
		$rowItemQuery = "SELECT
						...
						sd.touch AS touch,
						SI.name AS item,
						sub_item.name AS sub_item_name,
                        MAX(sd.created_at) as createdAt
					FROM ready_for_sale_detail sd
                    LEFT JOIN ready_for_sale sl ON sl.id = sd.sale_id
					LEFT JOIN item SI ON SI.id = sd.item_id
					LEFT JOIN sub_item ON sd.sub_item_id = sub_item.id
					WHERE $searchQuery
					GROUP BY sd.item_id
					ORDER BY sl.id DESC";

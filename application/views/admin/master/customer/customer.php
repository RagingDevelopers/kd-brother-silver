						...
						<form class="row" action="<?= (isset($update)) ? base_url("registration/customer/index/update/{$update['id']}") : base_url('registration/customer/index/store') ?>" method="post">
							...
							<div class="row mt-3">
								...
								<div class="col-sm-3 pt-3" id="processDropdown" style="display: none;">
									...
									<select class="form-select select2 " name="process_id" id="process_id">
										<option>Select Process</option>
										<?php
										$process = $this->db->get('process')->result();
										foreach ($process as $value) {
										?>
											<option value="<?= $value->id; ?>" <?php if (isset($update) && $value->id == $update['process_id']) {
																					echo 'selected';
																				} ?>><?= $value->name; ?>
											</option>
										<?php } ?>
...
<script class="javascript">
	...
	$(document).ready(function() {
		...
		$("#add").click(function() {
			...
			$('.append-here tr').last().find('.item_id').select2('open');
		});

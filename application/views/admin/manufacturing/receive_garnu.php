				...
				<tbody id="ReceivedBody">
					...
					foreach ($receivedData as $row) {
						if ($row['id'] > 0) {
							...
							$rm_string_array = [];
							$RmFinalLabour = 0;
							$selectedRawMaterialLotId = 0;
							foreach ($rawMaterial as $rm) {
								if ($selectedRawMaterialLotId <= 0 && !empty($rm['lot_wise_rm_id'])) {
									$selectedRawMaterialLotId = (int) $rm['lot_wise_rm_id'];
								}
							...
							$row['lot_wise_rm_id'] = $selectedRawMaterialLotId;
						}
					?>

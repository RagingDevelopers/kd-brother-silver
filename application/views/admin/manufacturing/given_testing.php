...
<div class="row">
	<div class="col-sm-12">
		<div class="card">
			<form action="<?= (isset($update)) ? base_url("manufacturing/given_testing/index/update/{$update['id']}") : base_url('manufacturing/given_testing/index/store') ?>" method="post" class="main-form" novalidate>
				<div class="card-header">
					<div class="card-status-top bg-blue"></div>
					<h1 class="card-title"><b> Given Testing </b></h1>
				</div>
						...
						<div class="row ms-1">
							<div class="row">
								...
								<div class="col-sm-3">
									...
									<select class="form-select worker_id" name="worker_id">
									...
									<table class="table card-table table-vcenter text-center text-nowrap ">
										...
										<tbody class="paste append-here">
											...
											foreach ($items as $row) { ?>
												<tr class="main-row">
													...
													<td>
														<select class="form-select dataType" data-metalId="<?= isset($row) ? $row['metal_type_id'] : null ?>" data-closingTouch="<?= isset($row) ? $row['closing_touch'] : null ?>" name="type[]">
															<option value="">Select Type</option>
															<option value="Metal" <?php if (isset($row['type']) && $row['type'] == 'Metal') {
																						echo 'selected';
																					} ?>>Metal</option>
															<option value="Row Material" <?php if (isset($row['type']) && $row['type'] == 'Row Material') {
																								echo 'selected';
																							} ?>>Row Material</option>
														</select>
													</td>
													<td>
														<select class="form-select metal_type_id" name="metal_type_id[]">
														</select>
													</td>
										...
										<tfoot>
											<tr class="border border-none">
												...
												<td>
													<label class="form-label"> Given Testing Weight(KG): </label>
												...
												<td>
													<label class="form-label">Given Testing Touch (%): </label>
													<div class="form-group input-icon">
...
<script class="javascript">
	var mainFunction = (function() {
		let main = {
			...
			ready: function() {
				...
				$(document).ready(function() {
					...
					$('.main-form').submit(function(e) {
						...
						validator
							.addField("#name", "Please enter Given name!");
			...
			validateSubmit: function(ref) {
				...
				// if (mainWeight < 1) {
				// 	return SweetAlert('warning', 'Given Weight must be greater then: 1'), preventEnter = true;
				// }
				if (metal_type < 1)
	...
	$(document).ready(function() {
		...
		$(document).on('change', '.dataType', function() {
			var type = $(this).val();
			var ref = $(this);
			var selected_id = ref.parents('tr').find(".dataType").attr('data-metalId');
			if (type == 'Metal') {
				ref.parents('tr').find(".metal_type_id").html("");
				$.ajax({
					type: "POST",
					showloader: true,
					dataType: "json",
					url: `${BaseUrl}manufacturing/given_testing/type`,
					data: {
					...
					success: function(response) {
						selected_id = null;
						if (response.success) {
							ref.parents('tr').find(".metal_type_id").html(response.data);
						} else {
							SweetAlert('warning', response.message);
						}
					},
				});
			} else if (type == 'Row Material') {
				ref.parents('tr').find(".metal_type_id").html("");
				$.ajax({
					type: "POST",
					showloader: true,
		...
		<?php if (isset($update)) { ?>
			$('.dataType').each(function(e) {
				$(this).trigger('change')
			});
		<?php } ?>
		$(document).on('change', '.metal_type_id', function() {
			var metal_type_id = $(this).val();
			var ref = $(this);
			if(!metal_type_id){
			<?php if (isset($update)) { ?>
				metal_type_id = ref.parents('tr').find(".dataType").attr('data-metalId');
			<?php } ?>
			}
			var type = ref.parents('tr').find(".dataType").val();
			var selected_id = ref.parents('tr').find(".dataType").attr('data-closingTouch');
			if (type == 'Metal') {
				ref.parents('tr').find(".closingTouch").html("");
				$.ajax({
					type: "POST",
				...
				$.ajax({
					...
					showloader: true,
					dataType: "json",
					url: `${BaseUrl}manufacturing/given_testing/rmClosing`,
					method: "POST",
					success: function(response) {
						if (response.success) {
							var getTouch = getOptions(response.data, selected_id);

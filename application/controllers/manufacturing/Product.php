...
class Product extends CI_Controller
{
	...
	const MainGivenView = "admin/manufacturing/main_given_report";
	...
	public function main_given_report($action = "", $id = null)
	{
		checkPrivilege(privilege["main_given_report"]);
		$page_data['page_title'] = 'Product Report';
		...
		$page_data['worker'] = $this->db->select('id,name')->get_where('customer', array('account_type_id' => 2))->result_array();
		$page_data['product'] = $this->db->select('id,name')->get('garnu')->result_array();
		return view(self::MainGivenView, $page_data);
	}
		...
		foreach ($records as $record) {
			...
			foreach ($given as $value) {
				...
				$data[] = array(
					'id' => $i,
					'garnu_name' => "
            					<a class='badge bg-indigo text-indigo-fg' target='_blank' href='" . base_url('manufacturing/process/manage/' . $id) . "'><b>$garnu_name</d></a>
            					<a class='bg-warning btn btn-action text-warning-fg me-2 ms-2 Received' data-demo-color data-bs-toggle='tooltip' data-bs-placement='top' data-garnu_id='$id' data-given_id='$given_id' data-bs-original-title='Received' href='#'>
									<i class='fa-brands fa-connectdevelop'></i>
								</a>",
					'given_date' => $value->creation_date,
					'process_name' => $value->process_name,
					'worker_name' => $value->customer_name,
					'given_qty' => $value->given_qty,
					'given_weight' => $value->given_weight,
					'given_rm_weight' => $value->row_material_weight,
					'given_total_weight' => $value->total_weight,
					'receive_qty' => $totalPcs,
					'receive_weight' => $totalWeight,
					'receive_rm_weight' => $totalRMWeight,
					'receive_total_weight' => $finalWeight,
				);
				$i = $i + 1;
			}
		}
